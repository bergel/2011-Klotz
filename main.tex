...
Klotz offers two dedicated layouts to easily exploit the third dimension to visualize data. The district layout organizes data as a city metaphor and the scatterplot layout organizes data along 3 axes.
...
\item containment is expressed with a \emph{view}, an object that enables the construction of the visualization by offering numerous methode.\ab{Is there more to say?}
\end{itemize}
...
\fig{h}{0.2}{figure2.png}{Magnitude class with 2 of his subclasses.}
...
The message \ct{numberOfInstanceVariables} is sent to each of \ct{Magnitude}'s subclasses. \ab{@@pass until here}

\ab{Can you adapt the example below with the Magnitude example. I like the idea to based our scripts on this example} Just like it happens with the \emph{nodes}, the \emph{edges} can be defined in 
...
affect every \emph{node}(\emph{edge}) defined before itself. \ab{I am not sure this example of colors is really interesting}
\begin{lstlisting}
| colors |
...
almost in linear proportion with the quantity of \emph{nodes}/\emph{edges}, 
wich is a great result, for a graphical interface. It also shown not
major difference between the computation of different kinds of \emph{Layouts}, 
even using \emph{edges}. This result seem to be perfect, but thats only 
because the quantity is still not demanding for the system.
