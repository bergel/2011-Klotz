...
Similarly with nodes edges are defined in one of four ways. The method \ct{edge:from:to:} and \ct{edges:from:to:} use default black and thin line shape offered by the view. Consider (\figref{figure4.png}):
...
Since no layout is specified, the default layout is horizontal line.
...
%view nodes: (Magnitude subclasses).	
%view edges: (Magnitude subclasses) from: #yourself to: (MetacelloVersionNumber)
%				using:(KLLine new width: [:cl | (cl numberOfMethods / 30) rounded])	
%\end{lstlisting}
...
Klotz exploits the third visual dimension to immerse the user an immersion 
